...
| `AAB_RESPONSIVE`        | (preset)    | Responsive viewport mode. When `true`, the page uses real window dimensions and reflows on resize. Desktop presets default to `true`; mobile/tablet default to `false`. Set to `false` to lock the viewport with `setDeviceMetricsOverride`. |
