...
describe('shouldDisableSandbox', () => {
  ...
  const origEnv = process.env.PILOT_CHROME_NO_SANDBOX;
  const origGetuid = process.getuid;

  afterEach(() => {
    Object.defineProperty(process, 'platform', { value: origPlatform });
    if (origEnv === undefined) {
      delete process.env.PILOT_CHROME_NO_SANDBOX;
    } else {
      process.env.PILOT_CHROME_NO_SANDBOX = origEnv;
    }
    if (origGetuid) {
  ...
  it('returns true when PILOT_CHROME_NO_SANDBOX=true', () => {
    process.env.PILOT_CHROME_NO_SANDBOX = 'true';
  ...
  it('returns true when PILOT_CHROME_NO_SANDBOX=1', () => {
    process.env.PILOT_CHROME_NO_SANDBOX = '1';
    expect(shouldDisableSandbox()).toBe(true);
  });

  it('returns false when PILOT_CHROME_NO_SANDBOX=false', () => {
    process.env.PILOT_CHROME_NO_SANDBOX = 'false';
    expect(shouldDisableSandbox()).toBe(false);
  });

  it('returns false when PILOT_CHROME_NO_SANDBOX is empty string', () => {
    process.env.PILOT_CHROME_NO_SANDBOX = '';
    expect(shouldDisableSandbox()).toBe(false);
  });
