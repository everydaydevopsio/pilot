...
describe('E2E: evaluate command', () => {
  it('returns document.title as a string', async () => {
    const res = await client.send('evaluate', { expression: 'document.title' });
    expect(res.ok).toBe(true);
...
describe('E2E: page_info command', () => {
  it('returns url, title, readyState', async () => {
    ...
    expect(['loading', 'interactive', 'complete']).toContain(result.readyState);
  });
});
...
describe('E2E: error handling', () => {
  it('returns unknown_method error', async () => {
    await expect(client.send('fly_to_moon')).rejects.toThrow('unknown_method');
  });
